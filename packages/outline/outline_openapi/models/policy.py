...
class Policy(BaseModel):
    ...
    abilities: Optional[Dict[str, Ability]] = Field(default=None, description="The abilities that are allowed by this policy, if an array is returned then the individual ID's in the array represent the memberships that grant the ability.")
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * OpenAPI `readOnly` fields are excluded.
        ...
        # override the default output from pydantic by calling `to_dict()` of each value in abilities (dict)
        _field_dict = {}
        if self.abilities:
            for _key_abilities in self.abilities:
                if self.abilities[_key_abilities]:
                    _field_dict[_key_abilities] = self.abilities[_key_abilities].to_dict()
            _dict['abilities'] = _field_dict
        return _dict
    ...
    def from_dict(cls, obj: Optional[Dict[str, Any]]) -> Optional[Self]:
        ...
        _obj = cls.model_validate({
            "id": obj.get("id"),
            "abilities": dict(
                (_k, Ability.from_dict(_v))
                for _k, _v in obj["abilities"].items()
            )
            if obj.get("abilities") is not None
            else None
        })
        return _obj
