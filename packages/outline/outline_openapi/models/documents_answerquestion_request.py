...
class DocumentsAnswerquestionRequest(BaseModel):
    """
    ...
    document_id: Optional[StrictStr] = Field(default=None, description="A document to search within", alias="documentId")
    status_filter: Optional[StrictStr] = Field(default=None, description="Any documents that are not in the specified status will be filtered out", alias="statusFilter")
    ...
    __properties: ClassVar[List[str]] = ["query", "userId", "collectionId", "documentId", "statusFilter", "dateFilter"]

    @field_validator('status_filter')
    def status_filter_validate_enum(cls, value):
        """Validates the enum"""
        if value is None:
            return value

        if value not in set(['draft', 'archived', 'published']):
            raise ValueError("must be one of enum values ('draft', 'archived', 'published')")
        return value
