...
class GroupsList200ResponseData(BaseModel):
    """
    GroupsList200ResponseData
    """ # noqa: E501
    groups: Optional[List[Group]] = None
    group_memberships: Optional[List[GroupMembership]] = Field(default=None, description="A preview of memberships in the group, note that this is not all memberships which can be queried from `groups.memberships`.", alias="groupMemberships")
    __properties: ClassVar[List[str]] = ["groups", "groupMemberships"]
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        * `None` is only added to the output dict for nullable fields that
        ...
        if self.group_memberships:
            for _item_group_memberships in self.group_memberships:
                if _item_group_memberships:
                    _items.append(_item_group_memberships.to_dict())
