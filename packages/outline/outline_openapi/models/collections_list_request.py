...
class CollectionsListRequest(BaseModel):
    ...
    query: Optional[StrictStr] = Field(default=None, description="If set, will filter the results by collection name.")
    status_filter: Optional[List[CollectionStatus]] = Field(default=None, description="An optional array of statuses to filter by.", alias="statusFilter")
    __properties: ClassVar[List[str]] = ["offset", "limit", "sort", "direction", "query", "statusFilter"]
